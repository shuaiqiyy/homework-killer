...
def user_infotmance(user_number,user_password):
    ...
    if code == 0:
        ...
        mid = da_lodin['data']['mid']
        name = da_lodin['data']['username']
        return code,hear,mid,name
    else:
        code = 401
        hear = mid = name = None
        return code,hear,mid,name

def class_infomance(hear,mid):
...
def homework_list_infomance(hear,mid,class_id,subject_id):
    ...
    data_homework_list = {
        'request': '{{"mid":{mid},"subject_id":{subjectid},"class_id":{classid},"page":1,"pageSize":10}}'.format(
            mid=mid,classid=class_id,subjectid=subject_id
            )
    }
